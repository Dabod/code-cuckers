...
public class Partida {
	...
	public void iniciarPartida() {
		...
		for (int x = 1; x <= 5; x++) {
			...
			while (nombreEquipo.equals(equipos[1].getNombreEquipo())
				...
				System.err.println(
						"�El nombre del equipo debe ser �nico! Los nombres no pueden repetirse, introduce otro nombre.");
				System.out.println("Equipo " + x + ": ");
				nombreEquipo = teclado.nextLine();
			}
	...
	public int[][] ronda(int[][] matDefAtk) {
		...
		for (equipoAtacante = 1; equipoAtacante < equipos.length; equipoAtacante++) {
			...
			if (equipos[equipoAtacante].getVidas() != 0) {
				...
				System.out.println("Ten en cuenta que esta decisi�n es definitiva (para esta ronda).");

				matDefAtk[0][equipoAtacante] = teclado.nextInt();

				while (matDefAtk[0][equipoAtacante] > 25 || matDefAtk[0][equipoAtacante] < 0) {
					System.err.println(
							"No puedes usar m�s de 25 misiles de defensa ni tener defensa negativa (�Los misiles de defensa cuestan x2!).");
					System.out.println("Misiles disponibles: " + equipos[equipoAtacante].getMisilesRonda() + ".\n");
					System.out.println("�Con cuantos misiles quieres defenderte?");
					System.out.println("Ten en cuenta que esta decisi�n es definitiva (para esta ronda).");
					matDefAtk[0][equipoAtacante] = teclado.nextInt();
				...
				do {
					...
					if (equipos[equipoAtacante].getMisilesRonda() != 0) {
						...
						do {
							System.out.println("\n�A quien quieres atacar? (Selecciona un n�mero de la lista)");
							...
							if (equipoObjetivo < 1 || equipoObjetivo > 5) {
								System.err.println("�Debes seleccionar un n�mero correspondiente a alg�n equipo!");
								...
								System.err.println("Ese equipo ya est� muerto. Selecciona otro objetivo.");
								equipoObjetivo = 0;
							}
						...
						while (matDefAtk[equipoObjetivo][equipoAtacante] > equipos[equipoAtacante].getMisilesRonda()
							...
							if (matDefAtk[equipoObjetivo][equipoAtacante] < 0) {
								System.err.println("No puedes atacar con un valor negativo, �est�s troleando o que?");
								System.out.println("�Con cuantos misiles vas a atacar a "
										+ equipos[equipoObjetivo].getNombreEquipo() + "?");
								System.out.println(
	...
	public void mostrarGanador() {
		...
		System.out.println("�El m�ximo campe�n mundial s�per guay de esta partida es... " + ganadorPartida
				+ "! �Sois loh mehore!");
	}
